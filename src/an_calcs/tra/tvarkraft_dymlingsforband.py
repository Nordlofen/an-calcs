    ...
    if movable_interlayer:
        ekvationer.extend(
            [
                ...
                _ekvation(r"b_{1,c.1} = \frac{\beta_F}{1 + \beta_F} \cdot \left(-(2 \cdot t_{il} + t_{1,eff} + t_{2,eff}) + \sqrt{(2 \cdot t_{il} + t_{1,eff} + t_{2,eff})^2 + \left(1 + \frac{1}{\beta_F}\right) \cdot (\delta_F \cdot t_{il}^2 + t_{1,eff}^2) + (1 + \beta_F) \cdot t_{2,eff}^2}\right)", "b1-delmode c.1, EC5:2025 Annex F Eq. F.44"),
                _ekvation(r"b_{1,c.2} = \frac{2 \cdot \beta_F}{2 + \beta_F} \cdot \left(-(t_{il} + t_{1,eff}/2) + \sqrt{(t_{il} + t_{1,eff}/2)^2 + \left(\frac{2}{\beta_F} + 1\right) \cdot \left(t_{1,eff}^2/4 + \delta_F \cdot t_{il}^2/4 + \frac{M_{y,Rk}}{k_{mod,1} \cdot f_{h,1,k} \cdot d}\right)}\right)", "b1-delmode c.2, EC5:2025 Annex F Eq. F.44"),
                _ekvation(r"b_{1,c.3} = \frac{2 \cdot \beta_F}{1 + 2 \cdot \beta_F} \cdot \left(-(t_{il} + t_{2,eff}/2) + \sqrt{(t_{il} + t_{2,eff}/2)^2 + t_{2,eff}^2/4 \cdot (1 + 2 \cdot \beta_F) + \left(\frac{1}{\beta_F} + 2\right) \cdot \left(\frac{M_{y,Rk}}{k_{mod,1} \cdot f_{h,1,k} \cdot d} + \delta_F \cdot t_{il}^2/4\right)}\right)", "b1-delmode c.3, EC5:2025 Annex F Eq. F.44"),
                _ekvation(r"b_{1,c.4} = \frac{1}{1 + \beta_F} \cdot \left(\beta_F \cdot t_{il} + \sqrt{\beta_F^2 \cdot t_{il}^2 + \beta_F \cdot (\beta_F + 1) \cdot \left(4 \cdot \frac{M_{y,Rk}}{k_{mod,1} \cdot f_{h,1,k} \cdot d} + \delta_F \cdot t_{il}^2/2\right)}\right)", "b1-delmode c.4, EC5:2025 Annex F Eq. F.44"),
...
tvarkraft_dymlingsforband.panel_schema = {
    ...
    "fields": [
        ...
        {"name": "d_h", "type": "float", "label": "Huvuddiameter", "symbol": "<i>d</i><sub>h</sub>", "unit": "mm", "default": 6.9, "visible_if": {"all": [{"field": "anslutningstyp", "not_equals": "stal-tra"}, {"field": "anslutningstyp", "not_equals": "tra-mellanlager-tra"}]}},
        {"name": "l", "type": "float", "label": "Längd", "symbol": "<i>l</i>", "unit": "mm", "default": 68.0},
        {"name": "l_gang", "type": "float", "label": "Gänglängd", "symbol": "<i>l</i><sub>gang</sub>", "unit": "mm", "default": 40.0, "visible_if": {"all": [{"field": "forbindartyp", "equals": "traskruv"}, {"field": "anslutningstyp", "not_equals": "tra-mellanlager-tra"}]}},
        {"name": "f_u", "type": "float", "label": "Draghållfasthet", "symbol": "<i>f</i><sub>u</sub>", "unit": "MPa", "default": 600.0, "visible_if": {"field": "M_y_Rk", "equals": 0.0}},
        {
            "name": "spiktyp",
            "type": "choice",
            "label": "Spiktyp",
        ...
        {"name": "slat_hals", "type": "bool", "label": "Slät hals", "symbol": "slät hals", "default": False, "visible_if": {"all": [{"field": "forbindartyp", "equals": "traskruv"}, {"field": "anslutningstyp", "not_equals": "tra-mellanlager-tra"}]}},
